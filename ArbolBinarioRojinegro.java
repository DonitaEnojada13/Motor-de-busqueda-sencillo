...
public class ArbolBinarioRojinegro<T extends Comparable<T>> implements ProcesadorPalabras{
	...
	public T elemento;
	public Color color;
	// cada nodo tendra una lista ligada de archivos
	// archivo1 -> archivo2 -> archivo3 -> null
	...
	public VerticeRn(T elemento) {
	    ...
	    this.archivos = new ListaLigada<Conteo>();
	}
	...
	if (s == null || s.isEmpty())
	    return;
	VerticeRn nodo = busca(raiz, (T)s);
	if (nodo != null) {
	    cambiaConteo(nodo, archivoActual);
	} else {
	    agregaRn((T)s);
	    cambiaConteo(ultimoAgregado, archivoActual);
	}
    }
	...
	if (a == null)
	    return null;
	int i = elem.compareTo(a.get());
	if(i == 0)
	    return a;
	return busca((i < 0) ? a.izquierdo : a.derecho, elem);
    }
    private void cambiaConteo(VerticeRn v, File ar) {
	if (v == null || ar == null)
	    return;
	for (Conteo nodo : v.archivos) {
	    if (nodo.documento.equals(ar)){
		nodo.actualiza();
		return;
	    }
	}
	Conteo registro = new Conteo(ar);
	v.archivos.agregarFinal(registro);
    }
